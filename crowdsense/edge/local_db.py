...
def initialize_db():
    ...
    conn.commit()
    conn.close()
    print(f"[DB] SQLite initialized at {SQLITE_DB_PATH}")
...
def get_unsynced_readings():
    conn = get_connection()
    rows = conn.execute(
        "SELECT * FROM crowd_readings_buffer WHERE synced = 0 ORDER BY captured_at ASC LIMIT 100"
    ).fetchall()
    conn.close()
...
def mark_synced(ids: list):
    ...
    conn.execute(
        f"UPDATE crowd_readings_buffer SET synced = 1 WHERE id IN ({placeholders})", ids
    )
    conn.commit()
    conn.close()
...
def get_stats():
    conn = get_connection()
    total   = conn.execute("SELECT COUNT(*) FROM crowd_readings_buffer").fetchone()[0]
    unsynced = conn.execute("SELECT COUNT(*) FROM crowd_readings_buffer WHERE synced = 0").fetchone()[0]
    conn.close()
